...
export default function MergeSortSimulator2(){
  ...
  const reset=()=>{ setArr([...base]); setInputValue(base.join(',')); setLogs([]); setLine(null); setComparing([]); setMerged([]); runningRef.current = false; setRunning(false); setPaused(false); pausedRef.current = false; };
  ...
  const execute=async ()=>{
    ...
    addLog('Starting merge sort'); setLine(1); await sleep(300);
    if(!runningRef.current){ addLog('Stopped'); setLine(null); setRunning(false); return; }
    const a=[...arr];

    const merge = async (l,m,r)=>{
      setLine(6); await sleep(100);
      let left=a.slice(l,m+1), right=a.slice(m+1,r+1), i=0,j=0,k=l;
      while(i<left.length && j<right.length){
        if(!runningRef.current){ addLog('Stopped'); setLine(null); setRunning(false); return; }
        setComparing([l+i, m+1+j]);
        addLog(`Comparing ${left[i]} and ${right[j]}`); setLine(6); await sleep(250);
        if(!runningRef.current){ addLog('Stopped'); setLine(null); setRunning(false); return; }
        if(left[i]<=right[j]){ a[k++]=left[i++]; } else { a[k++]=right[j++]; }
        setMerged([k-1]);
        setArr([...a]); await sleep(150);
      }
      while(i<left.length){ if(!runningRef.current){ addLog('Stopped'); setLine(null); setRunning(false); return; } setMerged([k]); a[k++]=left[i++]; setArr([...a]); await sleep(120); }
      while(j<right.length){ if(!runningRef.current){ addLog('Stopped'); setLine(null); setRunning(false); return; } setMerged([k]); a[k++]=right[j++]; setArr([...a]); await sleep(120); }
      setComparing([]);
      setMerged([]);
    };
    ...
    <div className="section">
      ...
      <div style={{display:'flex',gap:4,alignItems:'flex-end',justifyContent:'center',marginBottom:16,height:250,padding:'20px 10px',background:'var(--bg-card)',borderRadius:8,border:'1px solid var(--border)'}}>
        {arr.map((v,i)=> {
          ...
          if (comparing.includes(i)) barColor = 'var(--accent-orange)';
          else if (merged.includes(i)) barColor = 'var(--accent-green)';
          return (
            <div key={i} style={{display:'flex',flexDirection:'column',alignItems:'center',flex:1,maxWidth:60}}>
              ...
              <div style={{
                ...
                borderRadius:4,
                transition:'all 0.35s ease',
                opacity: (comparing.includes(i) || merged.includes(i)) ? 1 : 0.7,
                transform: (comparing.includes(i) || merged.includes(i)) ? 'scale(1.05)' : 'scale(1)'
              }}></div>
